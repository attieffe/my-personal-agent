# SINAPPS

Area per l’agenzia **SINAPPS** (società **T PROJECT SRL**) — sviluppo WordPress custom.

## Contesto
- **Società**: T PROJECT SRL
- **Brand agenzia**: Sinapps
- **Collaborazione dal**: inizio attività freelance di Attilio
- **Tipologia**: inizialmente sviluppo Android, poi sviluppo custom WordPress (plugin, temi, integrazioni)
- **Interlocutore principale**: Marco Viganò

## Referenti (da tenere aggiornati)
- **Marco Viganò** (socio/amministratore — PM e punto di riferimento commerciale)
- **Alessio Santamaria** (grafico/front-end)
- **Andrea Migliavacca**
- **Luca Proserpio** (sviluppo su progetti diversi)

## Progetti attivi
- [[PROGETTI/PROJ-Sinapps-blancone-20240101/README|BLANCONE]] — attivo dal 2024
- [[PROGETTI/PROJ-Sinapps-gosetups-20240101/README|GOSETUPS]] — attivo dal 2024
- [[PROGETTI/PROJ-Sinapps-nanosilk-20250101/README|NANOSILK]] — plugin gestione agenti, consegnato aprile 2026
- [[PROGETTI/PROJ-Sinapps-openfiber-20260521/README|OPENFIBER]] — sviluppo custom per cliente OpenFiber, attivo dal 2026-05-21

## TODO / Backlog
- [ ] Sentire Viga per lista ambienti, caricamento, pulizia automatica log nei miei plugin — vedere anche [[PROGETTI/README|PROGETTI/README]]

## ARCHIVIATI

- [[ARCHIVIATI|Archivio task chiusi]]

## Struttura
- [[PROGETTI/README|PROGETTI]]: sottocartelle per i progetti
- `CHANGELOG.md`: aggiornamenti del ramo
